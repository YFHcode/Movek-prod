...
import { approveClient } from "@/app/actions/admin_users";
...
export default function AdminClientsPage({ clients }: { clients: ClientRow[] }) {
    const [isPending, startTransition] = useTransition();

    const handleToggleApproval = (id: string, currentStatus: boolean) => {
        startTransition(async () => {
            const result = await approveClient(id, !currentStatus);
    ...
    const handleExport = () => {
        const exportData = clients.map(c => ({
            "ID": c.id,
            "Nom Complet": c.nom_prenom,
            "Entreprise": c.entreprise,
            "Email": c.email,
            "Téléphone": c.telephone || "",
            "Pays": c.pays || "",
            "Statut": c.is_approved ? "Approuvé" : "En attente",
            "Date d'inscription": format(new Date(c.created_at), "dd/MM/yyyy HH:mm")
        }));
        downloadCSV(exportData, "clients_movek");
    };
            ...
            cell: ({ row }) => {
                ...
                return (
                    <div className="flex items-center gap-2">
                        <button
                            onClick={() => handleToggleApproval(client.id, client.is_approved)}
                            disabled={isPending}
                            className={`flex items-center justify-center w-8 h-8 rounded shrink-0 transition-colors ${client.is_approved
                                ? "bg-red-500/10 text-red-500 hover:bg-red-500/20 border border-red-500/20"
                                : "bg-green-500/10 text-green-500 hover:bg-green-500/20 border border-green-500/20"
                                }`}
                            title={client.is_approved ? "Révoquer l'accès" : "Approuver le client"}
                        >
                            {isPending ? (
    ...
    return (
        <>
            <AdminTopBar title="Gestion des Clients" adminName="Admin" />
