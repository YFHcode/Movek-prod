...
import { loginFournisseur } from "@/app/actions/fournisseur_auth";
...
export default function FournisseurLoginPage() {
    const [state, formAction] = useFormState(loginFournisseur, initialState);
    const [showPassword, setShowPassword] = useState(false);
    const searchParams = useSearchParams();
    const redirectTo = searchParams.get("redirect") || "/espace-fournisseur";
            ...
            <div className="w-full max-w-[480px]">
                <div className="rounded-xl border border-movek-border bg-movek-card p-6 sm:p-8">
                            ...
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="mt-1 text-xs text-movek-text-secondary">
                                        Votre compte Fournisseur est en cours de révision par notre équipe
                                        ETREND. Vous recevrez un email dès que votre accès sera activé.
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <p className="mt-6 text-center text-sm text-movek-text-secondary">
                        Vous n&apos;êtes pas encore fournisseur ?{" "}
                        <Link href="/fournisseur/register" className="font-semibold text-movek-orange hover:underline">
